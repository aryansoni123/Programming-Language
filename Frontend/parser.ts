import { Stmt, Program, BinaryExpression, Identifier, NumericLiteral, Expression, CallExpression, MemberExpression, NullLiteral, VarDeclaration, AssignmentExpression, Property, ObjectLiteral } from "./ast.ts";
...
export default class Parser {   
    ...
    private parse_multiplicative_Expr(): Expression {
        let left = this.parse_call_mem_Expr();

        while (
            this.at().value == '*' || this.at().value == '/' || this.at().value == '%'
        ){
            const operator = this.eat().value
            const right = this.parse_call_mem_Expr();
    ...
    private parse_call_mem_Expr(): Expression {
        const member = this.parse_mem_Expr();
    ...
    private parse_call_Expr(caller: Expression): Expression {
        let call_expr: Expression = {
            ...
            caller
        } as CallExpression; // Watch out here
    ...
    private parse_args(): Expression[] {
        ...
        this.expect(TokenType.CloseParen, "Expected ) to end argument list.");
    ...
    private parse_args_list(): Expression[] {
        const args = [this.parse_assign_Expr()];
    ...
    private parse_mem_Expr(): Expression {
